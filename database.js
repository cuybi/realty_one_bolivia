...
function saveData() {
  try {
    ...
    // 1. Escribir primero en archivo temporal para asegurar escritura completa
    fs.writeFileSync(TMP_FILE, jsonStr, 'utf8');
    // 2. Renombrar atómicamente al archivo definitivo (evita archivos a medio escribir)
    fs.renameSync(TMP_FILE, DATA_FILE);
    // 3. Crear copia de respaldo (.bak) para recuperación en caliente
    try {
      fs.copyFileSync(DATA_FILE, BACKUP_FILE);
    } catch (be) {}
  } catch (err) {
    console.error("❌ Error en persistencia atómica de database.js:", err.message);
  }
}
